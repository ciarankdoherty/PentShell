...
logfile = "/opt/pentshell/log.txt"
historyfile = ".pentshell_history"
...
def main():
    ...
    log("Pentshell Opened by {0} on {1}".format(datetime.datetime.now(),str(os.getlogin())))
    while 1:
        try:
            comm = raw_input("{0}{1}@{2}pentshell;>{3} ".format(colors.purple,str(os.getlogin()),colors.grey,colors.end))
        ...
        if comm.lower().strip() != "":
                ...
                elif comm.lower().strip() == "exit": print(colors.OK + " Thanks for using Pentshell, Goodbye.");sys.exit()
                else:
                    if proxy == 1:
                        out = pcomm(comm)
                        print out
